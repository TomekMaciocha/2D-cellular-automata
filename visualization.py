...
class Visualization:
    def __init__(
        ...
        self.advanced_settings = input('Use advanced settings for making visualisation? (\'Y\' or \'y\' count as yes; any other as no):\n' )
        self.max_len = 0
    ...
    def set_figsize(self)->None:
        print("Set size of the square figure used for generating frames:")
        while True:
            tmp = float(input())
            if tmp <= 0:
    ...
    def set_fps(self)->None:
        print("Set number of frames per second in the animation (1 to 1000):")
        while True:
            tmp = float(input())
    ...
    def set_nsteps(self)->None:
        print('Set number of frames in the animation (1 to '+str(self.max_len)+'):')
        while True:
            tmp = int(input())
            if tmp <1 or tmp >self.max_len:
                print("Input not in specified range.")
                continue
            self.nsteps = tmp
            break
    ...
    def animate(self) -> None:
        ...
        ani.save('visualizations/'+ str(input('Set the filename of the animation:'))+'.gif')
        ani


# creating animation of the simulation
